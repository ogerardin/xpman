...
- **CircleCI** builds on Linux, macOS, and Windows **on `main` only** (branch-filtered in `.circleci/config.yml`); feature branches and PRs never trigger it. It is a **backup CI** and publishes **no releases**: its `publish-github-release` job is kept but disabled by default via the `pipeline.parameters.publish-release` boolean (default `false`, checked in an expression-based `filters` on the workflow entry) — it only runs if a pipeline is explicitly triggered with `{"parameters": {"publish-release": true}}`.
- **GitHub Actions** is the primary CI and release publisher (see `.github/workflows/build.yml`), building the same 3 OSes on `push` to `main` and on `pull_request`. On every `main` push it also creates/recreates the GitHub release: the `release` job (after `linux`/`windows`/`mac`) downloads the artifacts, determines the version via `mvn help:evaluate` (uses the runner's default JDK — no JDK 25 needed there), and runs `gh release create "$VERSION" ... --generate-notes` (delete-then-recreate, tag = `$VERSION`, same semantics as the old `ghr -delete`). It is the pre-merge CI for feature branches/PRs. Runner toolchains (Maven, JDK 25, ImageMagick, Inno Setup, WiX, Python) are pre-installed on the images; JDK 25 is wired via runner env vars (`JAVA_HOME_25_X64` / `JAVA_HOME_25_arm64`) written to `GITHUB_ENV`/`GITHUB_PATH` (the `env` context does not expose them). **Temporary test-lane convention:** a test branch can be temporarily added to `on.push.branches` (e.g. `feat/github-actions`) to run GHA on it in isolation — remember to remove it before merging. Never use `[skip ci]`/`[ci skip]` markers: they skip **all** three CIs.
